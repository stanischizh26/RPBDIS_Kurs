...
public class ProductTypeControllerTests
{
    ...
    [Fact]
    public async Task Get_ReturnsListOfProductTypes()
    {
        // Arrange
        var productTypes = new List<ProductTypeDto> { new(), new() };

        _mediatorMock
            .Setup(m => m.Send(new GetProductTypesQuery(), CancellationToken.None))
        ...
        var value = okResult?.Value as List<ProductTypeDto>;
        value.Should().HaveCount(2);
        value.Should().BeEquivalentTo(productTypes);

        _mediatorMock.Verify(m => m.Send(new GetProductTypesQuery(), CancellationToken.None), Times.Once);
    }
