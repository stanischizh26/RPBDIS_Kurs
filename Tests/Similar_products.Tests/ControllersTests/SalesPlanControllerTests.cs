...
public class SalesPlanControllerTests
{
    ...
    [Fact]
    public async Task Get_ReturnsListOfSalesPlans()
    {
        // Arrange
        var salesPlans = new List<SalesPlanDto> { new(), new() };

        _mediatorMock
            .Setup(m => m.Send(new GetSalesPlansQuery(), CancellationToken.None))
            .ReturnsAsync(salesPlans);
        ...
        var value = okResult?.Value as List<SalesPlanDto>;
        value.Should().HaveCount(2);
        value.Should().BeEquivalentTo(salesPlans);

        _mediatorMock.Verify(m => m.Send(new GetSalesPlansQuery(), CancellationToken.None), Times.Once);
    }
