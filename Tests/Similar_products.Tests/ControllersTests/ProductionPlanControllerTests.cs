...
public class ProductionPlanControllerTests
{
    ...
    [Fact]
    public async Task Get_ReturnsListOfProductionPlans()
    {
        // Arrange
        var productionPlans = new List<ProductionPlanDto> { new(), new() };

        _mediatorMock
            .Setup(m => m.Send(new GetProductionPlansQuery(), CancellationToken.None))
            .ReturnsAsync(productionPlans);
        ...
        var value = okResult?.Value as List<ProductionPlanDto>;
        value.Should().HaveCount(2);
        value.Should().BeEquivalentTo(productionPlans);

        _mediatorMock.Verify(m => m.Send(new GetProductionPlansQuery(), CancellationToken.None), Times.Once);
    }
