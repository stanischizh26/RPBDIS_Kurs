function editRowEnterprise(editButton) {
    ...
    if (isEditing) {
        ...
        saveChangesEnterprise(id, updatedData, row);
    } else {
        ...
        cancelButton.onclick = () => cancelEditingEnterprise(row);
        row.querySelector('td.actions').appendChild(cancelButton); // Кнопка отмены только в actions
    }
}

async function saveChangesEnterprise(id, updatedData, row) {
    try {
...
function cancelEditingEnterprise(row) {
    const cells = row.querySelectorAll('td[contenteditable]');
